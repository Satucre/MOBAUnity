...
////// Emissive:
                ...
                float3 emissive = (lerp( pow(saturate(lerp( (_MainTex_var.r+_MainTex_var.g+_MainTex_var.b), (_MainTex_var.r*_MainTex_var.g*_MainTex_var.b), _simpleToon )),_baseTexDensity), pow(_MainTex_var.rgb,_baseTexDensity), _useTexColor )*_emissive*i.vertexColor.rgb*((pow(lerp( (_MainTex_var.r*_MainTex_var.g*_MainTex_var.b), _MainTex_var.a, _haveAlpha ),_alphaDensity)*_alphaPower)*pow(_maskTex_var.r,_texMaskDensity)*saturate(pow((i.vertexColor.r+i.vertexColor.g+i.vertexColor.b),_vertColorDensity))*lerp( lerp( node_3491, (1.0 - node_3491), _fresInv ), 1.0, _fresnelOff )));
